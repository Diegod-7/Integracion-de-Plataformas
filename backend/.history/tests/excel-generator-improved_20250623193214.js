...
function generateExcelReport() {
  ...
  // APLICAR FORMATO Y COLORES PROFESIONALES
  
  // === HOJA DE CONTROL - FORMATO ===
  if (!wsHojaControl['!cols']) wsHojaControl['!cols'] = [];
  wsHojaControl['!cols'][0] = { wch: 20 }; // Ancho de columna A
  wsHojaControl['!cols'][1] = { wch: 35 }; // Ancho de columna B
  wsHojaControl['!cols'][2] = { wch: 15 }; // Ancho de columna C
  wsHojaControl['!cols'][3] = { wch: 25 }; // Ancho de columna D
  wsHojaControl['!cols'][4] = { wch: 15 }; // Ancho de columna E

  // Aplicar estilos a celdas específicas de la Hoja de Control
  const styleHeaderControl = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 14 },
    fill: { fgColor: { rgb: "2E4A6B" } }, // Azul corporativo oscuro
  ...
  const styleProjectTitle = {
    font: { bold: true, color: { rgb: "2E4A6B" }, size: 16 },
    alignment: { horizontal: "center", vertical: "center" }
  };

  const styleSectionHeader = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 12 },
    fill: { fgColor: { rgb: "4A6B8A" } }, // Azul corporativo medio
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "thin", color: { rgb: "000000" } },
      bottom: { style: "thin", color: { rgb: "000000" } },
  ...
  const styleDataLabel = {
    font: { bold: true, color: { rgb: "2E4A6B" } },
    fill: { fgColor: { rgb: "E8F0F8" } }, // Azul muy claro
    border: {
  ...
  const styleApproved = {
    ...
    fill: { fgColor: { rgb: "28A745" } }, // Verde aprobado
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "thin", color: { rgb: "000000" } },
      bottom: { style: "thin", color: { rgb: "000000" } },
  ...
  // === HOJA DE PRUEBAS - FORMATO ===
  if (!wsPruebas['!cols']) wsPruebas['!cols'] = [];
  wsPruebas['!cols'][0] = { wch: 12 }; // Paso
  wsPruebas['!cols'][1] = { wch: 45 }; // Descripción
  wsPruebas['!cols'][2] = { wch: 40 }; // Datos Entrada
  wsPruebas['!cols'][3] = { wch: 35 }; // Salida Esperada
  wsPruebas['!cols'][4] = { wch: 8 };  // ¿OK?
  wsPruebas['!cols'][5] = { wch: 40 }; // Observaciones
  wsPruebas['!cols'][6] = { wch: 12 }; // Tiempo

  const styleCaseHeader = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 12 },
    fill: { fgColor: { rgb: "DC3545" } }, // Rojo corporativo para casos
    alignment: { horizontal: "center", vertical: "center" },
    border: {
  ...
  const styleStepHeader = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 11 },
    fill: { fgColor: { rgb: "6C757D" } }, // Gris para headers de pasos
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "thin", color: { rgb: "000000" } },
      bottom: { style: "thin", color: { rgb: "000000" } },
  ...
  const styleStepData = {
    font: { color: { rgb: "333333" } },
    fill: { fgColor: { rgb: "F8F9FA" } }, // Gris muy claro alternado
    border: {
  ...
  const styleSuccess = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 14 },
    fill: { fgColor: { rgb: "28A745" } }, // Verde éxito
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "thin", color: { rgb: "000000" } },
      bottom: { style: "thin", color: { rgb: "000000" } },
  ...
  const styleResumen = {
    ...
    fill: { fgColor: { rgb: "17A2B8" } }, // Azul info para resumen
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "medium", color: { rgb: "000000" } },
      bottom: { style: "medium", color: { rgb: "000000" } },
  ...
  const styleConclusiones = {
    font: { bold: true, color: { rgb: "FFFFFF" }, size: 12 },
    fill: { fgColor: { rgb: "FFC107" } }, // Amarillo/naranja para conclusiones
    alignment: { horizontal: "center", vertical: "center" },
    border: {
      top: { style: "medium", color: { rgb: "000000" } },
      bottom: { style: "medium", color: { rgb: "000000" } },
  ...
  // Aplicar estilos básicos (Excel básico no soporta todos los estilos avanzados)
  // Nota: XLSX básico tiene limitaciones de formato, pero aplicaremos lo que podemos
  ...
  const fileName = `Plan_Pruebas_Integracion_Diego_Diaz_FORMAL_${timestamp}.xlsx`;
  const filePath = path.join(reportsDir, fileName);
  
  XLSX.writeFile(workbook, filePath);
  
  console.log(`✅ Reporte Excel FORMAL CON COLORES generado exitosamente:`);
  ...
  console.log(`   • Azul corporativo (#2E4A6B) - Headers principales`);
  console.log(`   • Verde éxito (#28A745) - Estados aprobados`);
  console.log(`   • Rojo corporativo (#DC3545) - Casos de prueba`);
